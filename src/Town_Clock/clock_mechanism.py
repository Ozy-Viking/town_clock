...
class ClockTower:
    def __init__(
        ...
        self.clocks: Clocks = Clocks(
            clock_pins=clock_pins, common_pin=common_pin, mode=self.mode
        )
        self.led = LED(pin=led_pin, mode=self.mode)
        self.position = position
        self.logger = Worker(name="Tower", clock=None)
        self.logger.log("debug", repr(self))
        self.next_sunset_sunrise_times: dict[int, float] = find_sunrise_sunset_times(
            **self.position
        )
        for t in self.next_sunset_sunrise_times:
            self.next_sunset_sunrise_times[t] = self.clock_time.mod_freq(
                tm=self.next_sunset_sunrise_times[t]
            )
        self.timezone: datetime.tzinfo = timezone_finder(**self.position)
        self.logger.log(
            "info", f"{self.timezone = }, {self.next_sunset_sunrise_times = }"
        )

    def __repr__(self) -> str:
        return f"ClockTower(pins = {self.pins}, mode = {self.mode})"

    def pulse(self, ct: ClockTime, clock_pulses: list[int] | None = None) -> None:
    ...
    def check_if_night(self, tm: float) -> None:
        if tm < self.next_sunset_sunrise_times[2]:
            ...
            self.next_sunset_sunrise_times[3] <= tm < self.next_sunset_sunrise_times[5]
        ):
            ...
            for t in self.next_sunset_sunrise_times:
                self.next_sunset_sunrise_times[t] = self.clock_time.mod_freq(
                    self.next_sunset_sunrise_times[t]
                )
            self.logger.log(
                "info", f"{self.timezone = }, {self.next_sunset_sunrise_times = }"
            )
