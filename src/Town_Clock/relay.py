...
class Relay:
    ...
    def __repr__(self) -> str:
        return (
            f"Relay(clock pins = {self.pin}, mode = {self.mode}," f" Name = {self.name}"
        )
    ...
    def double_pulse(self, clock_relay) -> bool:
        try:
            self.turn_on()
            clock_relay.turn_on()
            time.sleep(0.1)
            ...
            self.relay_log.log(
                "info", f"Pulsed relay: {self.name} and {clock_relay.name}"
            )
            return True
        except Exception as err:
            self.relay_log.log("error", f"{self.name} error: {err}")
...
class LED(Relay):
    def __init__(self, pin, name="LED", clock=None, mode: Mode = Mode.TEST) -> None:
        super().__init__(pin, name, clock, mode=mode)


class Clocks:
    def __init__(self, clock_pins, common_pin, mode: Mode = Mode.TEST) -> None:
        self.mode = mode
    ...
    def __repr__(self) -> str:
        return (
            f"Relay(clock pins = {self.clock_pins}, mode = {self.mode},"
            f" Number of clocks = {self.qty_clock})"
        )

    def pulse(self, clock: Clock | int = Clock.ALL) -> bool:
